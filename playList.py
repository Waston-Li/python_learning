...
#根据值删除元素.remove
moto=['ducati','yamaha','suziki']
too_expensive='ducati' ; moto.remove(too_expensive) #也可以写为moto.remove('ducati')

#组织列表
moto.sort()  #sorted(x)
print('.sort()使表中元素按字母顺序排列:',end=""); print(moto)
#此方法修改表内元素顺序 ，若想不修改的按顺序输出可以使用 sorted(x)
# .sort(reverse=True)可以按字母反序排列表中的元素
print("len()返回列表长度："+str(len(moto)))
